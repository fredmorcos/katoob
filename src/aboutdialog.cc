...
AboutDialog::AboutDialog() {
  ...
  try {
    Glib::RefPtr<Gdk::Pixbuf> icon =
        Gdk::Pixbuf::create_from_file(Utils::get_data_path("katoob.svg"));
    set_logo(icon);
  } catch (Glib::Exception &e) {
    std::cerr << "Error loading icon for about dialog: " << e.what()
              << std::endl;
  }

  std::stringstream license;

  license << _("Katoob is free software; you can redistribute it "
  ...
  set_comments(
      _("A lightweight multilingual unicode and bidi aware text editor"));
