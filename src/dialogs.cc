...
void katoob_error(const std::string &message) {
  Gtk::MessageDialog dlg(message, false, Gtk::MESSAGE_ERROR, Gtk::BUTTONS_OK,
                         true);
  dlg.run();
  //  dlg.destroy();
}
...
void katoob_info(const std::string &message) {
  Gtk::MessageDialog dlg(message, false, Gtk::MESSAGE_INFO, Gtk::BUTTONS_OK,
                         true);
  dlg.run();
}
...
bool katoob_big_info(const std::string &message, bool enable_insert) {
  ...
  sw.add(tv);
  dlg.get_content_area()->pack_start(sw, true, true);
...
int katoob_goto_dialog() {
  ...
  auto adjustment(Gtk::Adjustment::create(1, 1, 1000000000));

  Gtk::SpinButton go_to(adjustment);
  hbox.pack_start(go_to);

  Gtk::Button *button = dialog.add_button(_("Jump to"), Gtk::RESPONSE_OK);
  dialog.add_button(_("Cancel"), Gtk::RESPONSE_CANCEL);
...
ActivityMeter::ActivityMeter() : dialog(_("Please wait..."), true) {
  loop = Glib::MainLoop::create();
  dialog.add_button(_("Cancel"), Gtk::RESPONSE_CANCEL);
  dialog.get_content_area()->pack_start(bar, true, true);
  bar.set_pulse_step(0.007);
  dialog.signal_response().connect(
      sigc::mem_fun(this, &ActivityMeter::signal_response_cb));
}
...
void ActivityMeter::run() {
  conn = Glib::signal_idle().connect(
      sigc::bind_return(sigc::mem_fun(bar, &Gtk::ProgressBar::pulse), true),
      Glib::PRIORITY_LOW);
  dialog.show_all();
  loop->run();
}
