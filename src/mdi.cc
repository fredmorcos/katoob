...
MDI::MDI(Conf &conf, Encodings &enc)
    : _conf(conf), _encodings(enc), page_setup(PageSetup::create(_conf)),
      settings(PrintSettings::create()) {
  signal_switch_page().connect(
      sigc::mem_fun(*this, &MDI::signal_switch_page_cb));
  // 1 minute.
  Glib::signal_timeout().connect(sigc::mem_fun(this, &MDI::autosave),
                                 1 * 60 * 1000);
}
...
void MDI::scan_temp() {
  ...
  if (!Utils::get_recovery_files(temps, error)) {
    katoob_error(Utils::substitute(
        _("Failed to scan for any autorecovery files: %s"), error));
    return;
  }

  if (temps.size() > 0) {
    if (katoob_simple_question(
            _("Some unrecovered files were found. Try to recover them ?"))) {
      for (std::map<std::string, std::string>::iterator iter = temps.begin();
           iter != temps.end(); iter++) {
        Document *doc = create_document();
        if (doc) {
          doc->set_text(iter->second);
          if (unlink(iter->first.c_str()) == -1) {
            std::cerr << "Failed to unlink " << iter->first << " "
...
void MDI::add_document(Document *doc, bool signals) {
  // This should be first so when we get the switch_page signal we'll find
  // something.
  ...
  signal_document_added.emit(doc->get_readonly(), doc->get_modified(),
                             doc->get_title());
  ...
  // This is here because we can't put it in connect_signals() because the
  // document is appended after connect_signals() We will connect to the label
  // directly bypassing the Document object.
  dynamic_cast<Label *>(get_tab_label(*children[get_current_page()]))
      ->signal_close_clicked.connect(sigc::bind<Document *>(
          sigc::mem_fun(this, &MDI::signal_document_label_close_clicked_cb),
          doc));
}
...
void MDI::open_cb() {
  FileDialog dlg(_("Please select a file for editing."), FILE_OPEN, -1, _conf,
                 _encodings);
  if (dlg.run()) {
...
void MDI::open_location_cb() {
  ...
  if (dialog.run(error)) {
    ...
    sigc::slot<void, bool, const std::string &> slot =
        sigc::bind(sigc::mem_fun(this, &MDI::signal_transfer_complete_cb), uri,
                   enc, to_active);
    std::string error;
...
void MDI::signal_transfer_complete_cb(bool st, const std::string &str,
                                      const std::string KATOOB_UNUSED(uri),
                                      int enc, bool to_active) {
  if (!st) {
    katoob_error(str);
    return;
  }
  ...
  if (dlg.run()) {
    ...
    for (unsigned int x = 0; x < files.size(); x++) {
      if (Utils::katoob_read(files[x], contents)) {
        ...
        if (tenc == -1) {
          katoob_error(Utils::substitute(
              _("Couldn't detect the encoding of %s"), files[x]));
        } else {
...
bool MDI::save(bool replace) {
  ...
  FileDialog dlg(_("Please select a file for saving."), FILE_SAVE,
                 doc->get_encoding(), _conf, _encodings);
  if (dlg.run()) {
    int enc = dlg.encoding();
    std::vector<std::string> files = dlg.get();
...
void MDI::page_setup_cb() {
  ...
  page_setup->reset(Gtk::run_page_setup_dialog(
      *(dynamic_cast<Gtk::Window *>(get_parent()->get_parent())),
      page_setup->get_page_setup(), settings));
}
...
bool MDI::close(int n) {
  ...
  if (doc->get_modified()) {
    std::string message;
    if (doc->has_file()) {
      message = Utils::substitute(_("The file %s is not saved, Save first ?"),
                                  doc->get_file());
    } else {
      message = Utils::substitute(_("\"%s\" is not saved, Save first?"),
                                  doc->get_title());
    }
...
void MDI::recent_cb(std::string &rfile) {
  ...
  if (!Glib::file_test(rfile.c_str(), Glib::FILE_TEST_EXISTS)) {
    katoob_error(
        Utils::substitute(_("The file \"%s\" doesn't exist."), rfile.c_str()));
    return;
  }
...
void MDI::revert_cb() {
  ...
  if (katoob_simple_question((Utils::substitute(
          _("This will close the current file \"%s\". Discard any changes done "
            "and reload the saved copy of the file. Are you sure you want to "
            "continue ?"),
          doc->get_file())))) {
    std::string message;
...
void MDI::close_all_cb() { close_all(); }
...
void MDI::replace_cb() {
  ...
  // Connect the signals.
  dialog.signal_find.connect(sigc::bind<ReplaceDialog *>(
      sigc::mem_fun(*this, &MDI::replace_dialog_signal_find_cb), &dialog));
  dialog.signal_replace.connect(sigc::bind<ReplaceDialog *>(
      sigc::mem_fun(*this, &MDI::replace_dialog_signal_replace_cb), &dialog));
  dialog.signal_replace_all.connect(sigc::bind<ReplaceDialog *>(
      sigc::mem_fun(*this, &MDI::replace_dialog_signal_replace_all_cb),
      &dialog));
  dialog.run();
...
void MDI::replace_dialog_signal_replace_all_cb(ReplaceDialog *dialog) {
  ...
  katoob_info(Utils::substitute(
      ngettext("Replaced %d occurence.", "Replaced %d occurences.", x), x));
}
...
void MDI::signal_switch_page_cb(Gtk::Widget *const &KATOOB_UNUSED(p),
                                const guint &n) {
  children[n]->emit_signals();
  signal_doc_activated.emit(n);
}

void MDI::activate(int x) { set_current_page(x); }

void MDI::connect_signals(Document *doc) {
  // NOTE: Connect new signals whenever the Document class gets new!
  doc->signal_modified_set.connect(
      sigc::mem_fun(this, &MDI::signal_document_modified_cb));
  doc->signal_can_undo.connect(
      sigc::mem_fun(this, &MDI::signal_document_can_undo_cb));
  doc->signal_can_redo.connect(
      sigc::mem_fun(this, &MDI::signal_document_can_redo_cb));
  doc->signal_readonly_set.connect(
      sigc::mem_fun(this, &MDI::signal_document_readonly_cb));
  doc->signal_file_changed.connect(
      sigc::mem_fun(this, &MDI::signal_document_file_changed_cb));
  doc->signal_cursor_moved.connect(
      sigc::mem_fun(this, &MDI::signal_document_cursor_moved_cb));
  ...
  doc->signal_title_changed.connect(
      sigc::mem_fun(this, &MDI::signal_document_title_changed_cb));

  doc->signal_auto_spell_set.connect(
      sigc::mem_fun(this, &MDI::signal_document_spell_enabled_cb));
  doc->signal_dictionary_changed.connect(
      sigc::mem_fun(this, &MDI::signal_dictionary_changed_cb));

  doc->signal_wrap_text_set.connect(
      sigc::mem_fun(this, &MDI::signal_document_wrap_text_cb));
  doc->signal_line_numbers_set.connect(
      sigc::mem_fun(this, &MDI::signal_document_line_numbers_cb));
  ...
  doc->signal_highlight_set.connect(
      sigc::mem_fun(this, &MDI::signal_document_highlight_cb));
...
void MDI::signal_document_dict_lookup_cb(std::string word) {
  ...
  sigc::slot<void, bool, const std::string &> slot = sigc::bind(
      sigc::mem_fun(this, &MDI::signal_dict_transfer_complete_cb), word);
  std::string error;
  if (!Network::add_transfer(uri, error, slot)) {
    katoob_error(error);
  }
...
void MDI::signal_dict_transfer_complete_cb(bool st, const std::string &str,
...
void MDI::export_cb(Export ex) {
  ...
  if (Glib::file_test(file, Glib::FILE_TEST_EXISTS)) {
    std::string message(Utils::substitute(
        _("Are you sure you want to overwrite the file %s ?"), file));
