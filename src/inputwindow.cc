...
InputWindow::InputWindow(): Gtk::Dialog(_("Input Window"), false, true), _type(INPUT_DISPLAY_NONE)
{
...
void InputWindow::emulator_rebuild()
{
  ...
  for (iter = _em.begin(); iter != _em.end(); iter++) {
    ...
    button->signal_clicked().connect(
        sigc::bind<unsigned>(sigc::mem_fun(*this, &InputWindow::button_clicked_cb), x));
...
void InputWindow::multipress_rebuild()
{
  ...
  for (iter = _mp.begin(); iter != _mp.end(); iter++) {
    ...
    for (unsigned y = 0; y < iter->second.size(); y++) {
      ...
      button->signal_clicked().connect(
          sigc::bind<unsigned>(sigc::mem_fun(*this, &InputWindow::button_clicked_cb), p));
      buttons.push_back(button);
